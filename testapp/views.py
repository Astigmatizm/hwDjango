...
def test_email(request):
    ...
    #
    #
    # em = EmailMessage(subject='Запрошнный вами файл',
    #                   body='Получите запрошенный вами файл',
    #                   to=['user@supersite.ru'])
    # em.attach_file(r'requirements.txt')
    # em.send()

    # context = {'user': 'Vasya Pupkin'}
    # s = render_to_string('email/letter.txt', context)
    # em = EmailMessage(subject='Оповещение', body=s, to=['Vasya@superPupkin.ru'])
    # em.send()
